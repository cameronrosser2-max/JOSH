...
def dm_worker():
    try:
        ...
        push("ERROR: instagrapi not installed.", "error")
        state["running"] = False
        socketio.emit("stats", get_stats())
        return
        ...
        for industry_key in industry_keys:
            ...
            for tag in tags:
                ...
                try:
                    ...
                    push(f"  #{tag} error: {e}", "warn")

            for acct in random.sample(list(batch.values()), len(batch)):
                if state["sent"] >= state["target"] or _stop_flag.is_set():
                    break
    ...
    if _stop_flag.is_set():
        ...
        push(f"🎉 Done! Sent {state['sent']} DMs to trade businesses.", "success")
