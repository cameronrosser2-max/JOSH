...
def dm_worker(target: int):
        ...
        for industry_key in industry_keys:
            ...
            for acct in accounts:
                ...
                try:
                    ...
                    push(f"✓ @{uname} ({ind_name})  [{state['sent']}/{target}]", "success")
                except RateLimitError:
    ...
    if _stop_flag.is_set():
        push(f"⏹ Stopped. Sent {state['sent']} DMs this session.", "warn")
    else:
...
def save_schedule_cfg(data: dict):
